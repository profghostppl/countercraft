...
def test_check_dmesg_detects_intel_vtd_enabled():
    with patch("anchorroot.modules.dma_auditor.run_command", return_value=_cmd_result("DMAR: IOMMU enabled\n")):
...
def test_check_dmesg_detects_amd_vi_enabled():
    ...
    with patch("anchorroot.modules.dma_auditor.run_command", return_value=_cmd_result(output)):
        assert DmaAuditor._check_dmesg() is True


def test_check_dmesg_subsystem_present_but_not_confirmed_enabled():
    with patch("anchorroot.modules.dma_auditor.run_command", return_value=_cmd_result("DMAR: table not found\n")):
        assert DmaAuditor._check_dmesg() is False


def test_check_dmesg_no_relevant_lines_is_inconclusive():
    with patch("anchorroot.modules.dma_auditor.run_command", return_value=_cmd_result("Linux version 6.5.0\n")):
...
def test_check_dmesg_falls_back_to_journalctl_on_permission_denied():
    ...
    with patch("anchorroot.modules.dma_auditor.run_command", side_effect=responses):
        assert DmaAuditor._check_dmesg() is True
